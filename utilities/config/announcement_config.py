...
async def announcement_setup(ctx, bot):
    def check(m):
    ...
    for i in range(6):
        await ctx.send(
            "**Please mention your announcement channel by using the # method**\nType cancel to cancel setup\nType skip to skip this step",
            delete_after=20)
        ...
        if msg.content.lower() == 'cancel':
            ...
            await ctx.send("Announcement channel skipped", delete_after=20)
            return True
        ...
        if channel_check != None:
            await ctx.send(
                f"Announcement channel has been updated to {channel_check.mention}!",
                delete_after=60)
            settings['announcement_channel'] = (str(new_id))
